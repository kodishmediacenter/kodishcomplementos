...
home = xbmcvfs.translatePath("special://xbmc/addons")
...
def legendei():
    os.makedirs(xbmcvfs.translatePath("special://home/userdata/addon_data/plugin.audio.podcasts"))
    deffontes = xbmcvfs.translatePath("special://xbmc/addons/Kodish.repo.store/legenda/settings.xml")
    destino = xbmcvfs.translatePath("special://home/userdata/addon_data/plugin.audio.podcasts/settings.xml")
...
def principal2():
    addir('[B]... Vc está Banido :-D !!!!!![/B] por usar addons banidos pela kodish team por gentileza consultar a lupa do grupo','',1000,icon,'','','','')
    xbmcplugin.endOfDirectory(addon_handle)
    
def principal4():
    addir('[B]... A OEM Build Não Respeita as Politicas da Kodish. Por Isso Foi Negado Acesso da Build a Kodish .... [/B]','',1000,icon,'','','','')
    addir('[B]... A Build Foi permanentemente Banida. Exija que Dev respeite art 21 da Kodish pois sabem bem o que é [/B]','',1000,icon,'','','','')
...
def principal32():
    ...
    addir('[B]Remover Oneplay TV Web              [/B]','',2003,icon,'','','','')

    addir('[B]Utilitarios                         [/B]','',2000,icon,'','','','')
    xbmcplugin.endOfDirectory(addon_handle)

def principal33():
    addir('[B]Calculadora                     [/B]','',1006,icon,'','','','')
    addir('[B]Tributação Remessa Conforme     [/B]','',2001,icon,'','','','')
    addir('[B]Enviar Log para Webinterface    [/B]','',2002,icon,'','','','')
    addir('[B]Sugestão de Podcast (Para Add-on Podcasts)                         [/B]','',1004,icon,'','','','')
    addir('[B]Criar addon Youtube Kodi            [/B]','',1003,icon,'','','','')
    addir('[B]Limpeza do Kodi                     [/B]','',1005,icon,'','','','')
    xbmcplugin.endOfDirectory(addon_handle)
...
def install_build(name,url,skin):
    if xbmcgui.Dialog().yesno(addonname, 'Deseja Instalar %s?\nA Configuração atual do kodi será modificada'%name):
        ...
        addons_id = re.sub('https://raw.githubusercontent.com/kodishmc/kodi19/master/addons/',r'',url).replace('.zip','')
...
def open_url(url):
    ...
    try:
        hdr = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36',
    ...
    if mode==None:
        for a in ['special://home/addons/KeltecMP.repository','special://home/addons/plugin.video.KelTec.MP.matrix','special://home/addons/plugin.program.keltec.wizard','special://home/userdata/addon_data/plugin.video.TorrentPlay']:
            ...
            if os.path.exists(existe)==True:
                ...
                for a in ['special://xbmc/addons/demo','special://xbmc/addons/KeltecMP.repository','special://xbmc/addons/plugin.video.KelTec.MP.matrix','special://xbmc/addons/plugin.program.keltec.wizard','special://xbmc/userdata/addon_data/plugin.video.TorrentPlay']:
                    existe = xbmcvfs.translatePath(a)
                    if os.path.exists(existe)==True:
                        principal4()
                    else:
        ...
        try:
            # Avalia a expressão matemática digitada pelo usuário
            resultado = eval(operacao)
            # Mostra o resultado em um diálogo
            xbmcgui.Dialog().ok('[B][COLOR white]Resultado[/COLOR][/B]', str(resultado))
        except Exception as e:
            # Mostra uma mensagem de erro caso a avaliação falhe
            xbmcgui.Dialog().ok('[B][COLOR red]Erro[/COLOR][/B]', 'Operação inválida: ' + str(e))
    elif mode==2000:
        principal33()
    elif mode==2001: 
        import tributacao
