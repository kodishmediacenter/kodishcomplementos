...
name_addon_info = 'plugin.video.Gif.Peg.Tv'
viewmode = None
tsdownloader = False
hlsretry = False
...
FONTE = 'aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL0dJRjU1My9NZW51cy9tYWluL01lbnU='
try:
        ...
        for other,channel_name,stream_url in match2:
            ...
            if cat:
                ...
                if not 'plugin' in stream_url and not 'User-Agent' in stream_url and not 'Referer' in stream_url and not 'Origin' in stream_url and not 'Cookie' in stream_url:
                    stream_url = stream_url + '|User-Agent=Android'
                if '.m3u8' in stream_url and f4mtester and not 'pluto.tv' in stream_url and not 'plugin' in stream_url:
                    plugin_m3u8 = 'plugin://plugin.video.f4mTester/?maxbitrate=0&buffer=2156&url={0}&streamtype=HLSRETRY&iconImage={1}&name= {2}[COLOR springgreen] GIF [COLOR springgreen]PEG [COLOR orangered]TV[/COLOR]'.format(urllib_parse.quote_plus(stream_url),urllib_parse.quote_plus(thumbnail),urllib_parse.quote_plus(channel_name))
                    ...
                    plugin_ts = 'plugin://plugin.video.f4mTester/?maxbitrate=0&buffer=2156&url={0}&streamtype=TSDOWNLOADER&iconImage={1}&name= {2}[COLOR springgreen] GIF [COLOR springgreen]PEG [COLOR orangered]TV[/COLOR]'.format(urllib_parse.quote_plus(stream_url),urllib_parse.quote_plus(thumbnail),urllib_parse.quote_plus(channel_name))
            ...
            if cat:
                if cat == name:
                    if not 'plugin' in stream_url and not 'User-Agent' in stream_url and not 'Referer' in stream_url and not 'Origin' in stream_url and not 'Cookie' in stream_url:
                        stream_url = stream_url + '|User-Agent=Android'
                    if '.m3u8' in stream_url and f4mtester and not 'pluto.tv' in stream_url and not 'plugin' in stream_url:
                        plugin_m3u8 = 'plugin://plugin.video.f4mTester/?maxbitrate=0&buffer=2156&url={0}&streamtype=HLSRETRY&iconImage={1}&name={2}'.format(urllib_parse.quote_plus(stream_url),urllib_parse.quote_plus(thumbnail),urllib_parse.quote_plus(channel_name))
                        stream_url = plugin_m3u8
                    elif f4mtester and not '?' in stream_url and not '.mp4' in stream_url and not '.mkv' in stream_url and not '.avi' in stream_url and not '.rmvb' in stream_url and not '.mp3' in stream_url and not '.wmv' in stream_url and not '.wma' in stream_url and not '.ac3' in stream_url and not 'pluto.tv' in stream_url and not 'plugin' in stream_url:
                        plugin_ts = 'plugin://plugin.video.f4mTester/?maxbitrate=0&buffer=2156&url={0}&streamtype=TSDOWNLOADER&iconImage={1}&name={2}'.format(urllib_parse.quote_plus(stream_url),urllib_parse.quote_plus(thumbnail),urllib_parse.quote_plus(channel_name))
                        stream_url = plugin_ts
                    addLink(stream_url, channel_name, thumbnail, '', '', '', '', '', None, '', total)
...
def parse_m3u(data):
    ...
    for other, channel_name, stream_url in match:
        ...
        if 'type' in other:
            ...
            stream_url = 'plugin://plugin.video.f4mTester/?maxbitrate=0&buffer=2156&url={0}&amp;streamtype=TSDOWNLOADER&name={1}'.format(urllib_parse.quote_plus(stream_url), urllib_parse.quote(channel_name))
        elif hlsretry and '.m3u8' in stream_url:
            stream_url = 'plugin://plugin.video.f4mTester/?maxbitrate=0&buffer=2156&url={0}&amp;streamtype=HLSRETRY&name={1}'.format(urllib_parse.quote_plus(stream_url), urllib_parse.quote(channel_name))
        addLink(stream_url, channel_name, thumbnail, '', '', '', '', '', None, '', total)
        ...
        try:
            ...
            if len(item.findall('link')) > 0:
                ...
                for i in item.findall('f4m'):
                    if i.text is not None:
                        if '.f4m' in i.text:
                            f4m = 'plugin://plugin.video.f4mTester/?maxbitrate=0&buffer=2156&url=' + urllib_parse.quote_plus(i.text)
                        elif '.m3u8' in i.text:
                            f4m = 'plugin://plugin.video.f4mTester/maxbitrate=0&buffer=2156&url=' + urllib_parse.quote_plus(i.text) + '&amp;streamtype=HLS'
                        else:
                            f4m = 'plugin://plugin.video.f4mTester/maxbitrate=0&buffer=2156&url=' + urllib_parse.quote_plus(i.text) + '&amp;streamtype=SIMPLE'
                    url.append(f4m)
